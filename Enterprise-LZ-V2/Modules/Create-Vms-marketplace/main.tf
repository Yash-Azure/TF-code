...
resource "azurerm_public_ip" "vm" {
  ...
  name                = "${var.vm_hostname}${format("%02d",count.index+1)}-pip-${count.index+1}"
  resource_group_name = data.azurerm_resource_group.vm.name
  location            = coalesce(var.location, data.azurerm_resource_group.vm.location)
...
resource "azurerm_network_interface" "vm" {
  ...
  ip_configuration {
    name                          = "${var.vm_hostname}${format("%02d",count.index+1)}-ip-${format("%02d",count.index)}"
    subnet_id                     = var.vnet_subnet_id
    private_ip_address_allocation = "Dynamic"

    public_ip_address_id          = length(azurerm_public_ip.vm.*.id) > 0 ? element(concat(azurerm_public_ip.vm.*.id, tolist([""])), count.index + 1): ""
  }
